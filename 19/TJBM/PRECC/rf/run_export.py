...
data = np.load(output_dir+'results'+opt+'.npz',allow_pickle=True)
...
difference = predict_y[:,0,:,:,0] - test_y[:,0,:,:,0]

data_dict = {label:test_y[:,0,:,:,0],
             label+'_ML_predicted':predict_y[:,0,:,:,0],
...
functions.export_data(test_feature_file, data_dict, label, 
                      output_dict, data['time'], output_dir, 'results'+opt+'.nc')
